...
transformed parameters {
  ...
  for (n in 1:N)
  {
    latent_rate[n] = 1E-15;    //some small number to keep it positive
    for (m in 1:M)
    {
      latent_rate[n]+= phi(age[m],0.1,13,beta)*SFH[n][m]/phi(1,0.1,13,beta); //sum the rate arising from each epoch
    }
