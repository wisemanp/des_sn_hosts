...
class Rates():
    ...
    def generate_sn_samples(self,mass_col='HOST_LOGMASS',err_col='HOST_LOGMASS_ERR',index_col = 'CIDint',n_iter=1E5,save_samples=True):
        ...
        if save_samples:
            savename=self.config['rates_root']+'data/'+os.path.split(self.SN_fn)[-1].replace('.csv','_mass_resampled.h5')
    ...
    def generate_field_samples(self,mass_col='mass',err_col='mass_err',index_col = 'id',n_iter=1E5,save_samples=True):
        ...
        if save_samples:
            savename=self.config['rates_root']+'data/'+os.path.split(self.field_fn)[-1].replace('.csv','_mass_resampled.h5')
