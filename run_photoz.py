    ...
    for f in args.fields:
        for ch in args.chips:
            cat_fn = os.path.join(config['des_root'],
                '5yr_stacks/MY%s/SN-%s/CAP/%s/%s_SN-%s_%s_obj_deep_v%s.cat'%(args.my,f,ch,my,f,ch,config['cat_version']))
            args.input = cat_fn
            args.output = '%s_%s_%s_%s_%s_%s'%(my,f,ch,config['cat_version'],config['params']['z_max'],config['params']['z_step'])
            photoz.main(args)
