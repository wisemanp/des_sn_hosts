...
def tripp_rv(alpha,beta,M0,sigma_int,mass_step,age_step,args):
    return M0 + args['distmod'] + norm(0,sigma_int).rvs(size=len(args['c'])) + beta*args['c_int'] - alpha*args['x1'] + (args['rv']+1)*args['E'] + \
           add_mass_step(np.log10(args['mass']),mass_step['mag'],mass_step['loc']) + add_age_step(args['SN_age']/1000,age_step['mag'],age_step['loc'])
...
def tripp_rv_two_beta_age(alpha,beta_young,beta_old,M0,sigma_int,mass_step,age_step,args):
    ...
    return M0 + args['distmod'] + norm(0, sigma_int).rvs(size=len(args['c'])) + beta * args['c_int'] - alpha * args[
        ...
        args['SN_age']/1000, age_step['mag'], age_step['loc'])
...
def tripp_rv_two_beta_popns_age(alpha,beta_young,sig_beta_young,beta_old,sig_beta_old,M0,sigma_int,mass_step,age_step,args):
    beta = (norm(beta_old,sig_beta_old).rvs(size=len(args['c'])) * (args['prog_age']=='old')) + (norm(beta_young,sig_beta_young).rvs(size=len(args['c'])) * (args['prog_age']=='young'))
