...
def sim_worker(args):
    ...
    alpha_young,alpha_old,mass_step,cfg = [args[i] for i in range(4)]
    pth = cfg['config_path']
    ...
    sim.sample_SNe(zarr,n_samples_arr,
                   ...
                   '%s_test_SN_sim_%.2f_%.2f_%.2f.h5'%(model_name,alpha_young,alpha_old,mass_step)))
    ...
    sim.sim_df.to_hdf(os.path.join('/media/data3/wiseman/des/AURA/sims/SNe/for_BBC/',cfg['save']['dir'],
        '%s_test_SN_sim_%.2f_%.2f_%.2f.h5'%(model_name,alpha_young,alpha_old,mass_step)),key='sim')
...
if __name__=='__main__':
    ...
    mass_step_grid = np.arange(cfg['mass_step']['lo'],cfg['mass_step']['hi'],cfg['mass_step']['step'])

    args = []
    for alpha_young in alpha_young_grid:
        for alpha_old in alpha_old_grid:
            for mass_step in mass_step_grid:
                args.append([alpha_old,alpha_young,mass_step,cfg])
    multi_sim(args)
    ...
    for alpha_young in alpha_young_grid:
        for alpha_old in alpha_old_grid:
            for mass_step in mass_step_grid:
                ...
                param_map[n] = [alpha_young,alpha_old,mass_step]
                n+=1
