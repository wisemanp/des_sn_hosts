...
def plot_mu_res(sim,obs=True,label_ext=''):
    ...
    for n,g in sim.sim_df.groupby(pd.cut(sim.sim_df['U-R'],bins=np.linspace(-0.5,2.5,30))):
        ax.scatter(n.mid,np.average(g['mu_res'],weights=np.std(g['mu_res'])/np.sqrt(len(g['mu_res']))),c='r',marker='D',s=100)
        ax.errorbar(n.mid,np.average(g['mu_res'],weights=np.std(g['mu_res'])/np.sqrt(len(g['mu_res']))),yerr=g['mu_res'].std()/np.sqrt(len(g['mu_res'])),c='r',marker=None,ls='none')
    for n,g in sim.sim_df.groupby(pd.cut(sim.sim_df['U-R'],bins=np.linspace(-0.5,2.5,3))):
        print(n.mid)
        ax.errorbar(n.mid,np.average(g['mu_res'],weights=np.std(g['mu_res'])/np.sqrt(len(g['mu_res']))),yerr=g['mu_res'].std()/np.sqrt(len(g['mu_res'])),c='c',marker='s',markersize=20,ls='none')
